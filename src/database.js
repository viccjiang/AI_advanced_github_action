...
// DATABASE_PATH 可指向獨立的資料庫檔案（測試用），未設定時使用專案根目錄的 database.sqlite
...
function initializeDatabase() {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS orders (
      ...
      shipping_method TEXT NOT NULL DEFAULT 'home_delivery' CHECK(shipping_method IN ('home_delivery', 'cvs_pickup')),
  ...
  // Migrations: add columns to existing databases (ignored when already present)
  const orderColumnMigrations = [
    // ECPay integration
    'ALTER TABLE orders ADD COLUMN merchant_trade_no TEXT',
    // Shipping fee breakdown
    'ALTER TABLE orders ADD COLUMN subtotal INTEGER NOT NULL DEFAULT 0',
    "ALTER TABLE orders ADD COLUMN shipping_method TEXT NOT NULL DEFAULT 'home_delivery'",
    'ALTER TABLE orders ADD COLUMN shipping_fee INTEGER NOT NULL DEFAULT 0',
    'ALTER TABLE orders ADD COLUMN is_remote INTEGER NOT NULL DEFAULT 0',
